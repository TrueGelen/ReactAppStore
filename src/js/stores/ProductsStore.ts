...
import { PhoneModel, TPhoneDescription } from "../models/PhoneModel";
import type { TProductData } from "../models/parentModels/ProductModel";
import type { ProductsApi } from "../serverApiModel/ProductsApi";
import { EN_LOC, RU_LOC } from "../utils/constants/constants";
    ...
    if (!isNil(serverProducts)) {
      forEach(serverProducts, (product) => {
        if (product.__typename === PhoneModel.typename) {
          ...
          products.push(new PhoneModel(product) as T);
        }
      });
    }
